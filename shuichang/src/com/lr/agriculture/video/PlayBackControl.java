...
public class PlayBackControl implements RtspClientCallback, PlayerDisplayCB {
	...
	private boolean checkParams(PlayBackParams obj) {
		...
		if (null == obj.surfaceView || null == obj.startTime
				|| null == obj.endTime || null == obj.url || null == obj.name
				|| null == obj.passwrod || obj.url.equals("")
				|| obj.name.equals("") || obj.passwrod.equals("")) {
	...
	private void startRtsp() {
		...
		mRtspEngineIndex = mRtspClientHandle.createRtspClientEngine(this,
				mModel);
		...
		boolean ret = mRtspClientHandle.playbackByTime(mRtspEngineIndex, mUrl,
				mName, mPassword, mStartTime, mEndTime);
		if (!ret) {
			...
			if (null == mPlayBackCallBack) {
				...
				DebugLog.error(TAG,
						"startRtsp()::playbackByTime() fail, errorCode is R"
								+ errorCode);
				mPlayBackCallBack
						.onMessageCallback(ConstantPlayBack.START_RTSP_FAIL);
				return;
			}
		}
		...
		if (null == mPlayBackCallBack) {
			...
			mPlayBackCallBack
					.onMessageCallback(ConstantPlayBack.START_RTSP_SUCCESS);
		}
	}
	...
	public void pausePlayBack() {
		if (PLAYBACK_PLAY != mPlayBackState) {
			if (mPlayBackCallBack != null) {
				mPlayBackCallBack
						.onMessageCallback(ConstantPlayBack.PAUSE_FAIL_NPLAY_STATE);
			}
			return;
		}
		...
		if (!ret) {
			...
			DebugLog.error(TAG,
					"pausePlayBack():: pause() fail, errorCode is P"
							+ errorCode);
			if (null == mPlayBackCallBack) {
				DebugLog.error(TAG,
						"pausePlayBack():: mPlayBackCallBack is null");
				return;
			} else {
				mPlayBackCallBack
						.onMessageCallback(ConstantPlayBack.PAUSE_FAIL);
				return;
			}
		}
		...
		if (!ret) {
			...
			mRtspClientHandle.releaseRtspClientEngineer(mRtspEngineIndex);
			DebugLog.error(TAG,
					"pausePlayBack():: pause() fail, errorCode is R"
							+ errorCode);
			if (null == mPlayBackCallBack) {
				DebugLog.error(TAG,
						"pausePlayBack():: mPlayBackCallBack is null");
	...
	public void resumePlayBack() {
		if (PLAYBACK_PAUSE != mPlayBackState) {
			if (mPlayBackCallBack != null) {
				mPlayBackCallBack
						.onMessageCallback(ConstantPlayBack.RESUEM_FAIL_NPAUSE_STATE);
			}
			return;
		}
		...
		if (!ret) {
			...
			mRtspClientHandle.releaseRtspClientEngineer(mRtspEngineIndex);
			DebugLog.error(TAG,
					"resumePlayBack():: pause() fail, errorCode is R"
							+ errorCode);
			if (null == mPlayBackCallBack) {
				DebugLog.error(TAG,
						"resumePlayBack():: mPlayBackCallBack is null");
				return;
			} else {
				mPlayBackCallBack
						.onMessageCallback(ConstantPlayBack.RESUEM_FAIL);
				return;
			}
		}
		...
		if (null == mPlayBackCallBack) {
			...
			mPlayBackCallBack
					.onMessageCallback(ConstantPlayBack.RESUEM_SUCCESS);
		}
	}
	...
	@Override
	public void onDataCallBack(int handle, int dataType, byte[] data,
			int length, int timeStamp, int packetNo, int useId) {
		mStreamRate += length;
		...
		case RtspClient.DATATYPE_HEADER:
			...
			if (!ret) {
				...
				if (null != mPlayBackCallBack) {
					mPlayBackCallBack
							.onMessageCallback(ConstantPlayBack.START_OPEN_FAILED);
					return;
				} else {
					DebugLog.error(TAG,
							"onDataCallBack():: mPlayBackCallBack is null");
				}
			} else {
	 ...
	 *            数据长度
	 */
	private void processRecordData(int dataType, byte[] dataBuffer,
			int dataLength) {
	...
	private boolean writeStreamData(byte[] recordData, int length) {
		if (null == recordData || length <= 0) {
			DebugLog.error(TAG,
					"writeStreamData() fail recordData is null or length <=0");
			return false;
		}
	...
	private boolean startPlayer(byte[] data, int len) {
		...
		boolean ret = mPlayerHandler.setStreamOpenMode(mPlayerPort,
				Player.STREAM_REALTIME);
		if (!ret) {
			int tempErrorCode = mPlayerHandler.getLastError(mPlayerPort);
			DebugLog.error(TAG,
					"startPlayer():: mPlayerHandler.setStreamOpenMode() failed, errorCode is P"
							+ tempErrorCode);
		...
		ret = mPlayerHandler
				.openStream(mPlayerPort, data, len, 2 * 1024 * 1024);
		if (!ret) {
	...
	private void closePlayer() {
		if (null != mPlayerHandler) {
			if (-1 != mPlayerPort) {
				...
				if (!ret) {
					DebugLog.error(TAG,
							"closePlayer(): Player closeStream  failed!");
				}

				ret = mPlayerHandler.freePort(mPlayerPort);
				if (!ret) {
					DebugLog.error(TAG,
							"closePlayer(): Player freePort  failed!");
				}
				mPlayerPort = -1;
			}
		}
	...
	public boolean capture(String filePath, String picName) {
		...
		if (PLAYBACK_PLAY != mPlayBackState) {
			if (null != mPlayBackCallBack) {
				mPlayBackCallBack
						.onMessageCallback(ConstantPlayBack.CAPTURE_FAILED_NPLAY_STATE);
			}
			return false;
		}
	...
	private byte[] getPictureOnJPEG() {
		...
		if (-1 == mPlayerPort) {
			DebugLog.error(TAG,
					"getPictureOnJPEG():: mPlayerPort is Unavailable");
			return new byte[0];
		}
		...
		boolean ret = mPlayerHandler.getJPEG(mPlayerPort, pictureBuffer,
				picSize, jpgSize);
		if (!ret) {
			DebugLog.error(TAG,
					"getPictureOnJPEG():: mPlayerHandler.getJPEG() return false，errorCode is P"
							+ mPlayerHandler.getLastError(mPlayerPort));
			return new byte[0];
		}
	...
	private boolean createRecordFile(String path, String fileName) {
		if (null == path || path.equals("") || null == fileName
				|| fileName.equals("")) {
			return false;
		}
	...
	/*
	 * handle - - 引擎id opt -
	 * -回调消息，包括：RTSPCLIENT_MSG_PLAYBACK_FINISH,RTSPCLIENT_MSG_BUFFER_OVERFLOW
	 * ,RTSPCLIENT_MSG_CONNECTION_EXCEPTION 三种 param1 - - 保留参数 param2 - - 保留参数
	 * useId - - 用户数据，默认就是引擎id与handle相同
	 */
	@Override
	public void onMessageCallBack(int handle, int opt, int param1, int param2,
		...
		if (opt == RtspClient.RTSPCLIENT_MSG_CONNECTION_EXCEPTION) {
			DebugLog.error(TAG,
					"onMessageCallBack():: RTSPCLIENT_MSG_CONNECTION_EXCEPTION");

			stopPlayBack();
			mPlayBackCallBack
					.onMessageCallback(RtspClient.RTSPCLIENT_MSG_CONNECTION_EXCEPTION);
		}
	}
	...
	public void onDisplay(int arg0, byte[] arg1, int arg2, int arg3, int arg4,
		...
		if (null != mPlayBackCallBack) {
			mPlayBackCallBack
					.onMessageCallback(ConstantPlayBack.PLAY_DISPLAY_SUCCESS);
		} else {
