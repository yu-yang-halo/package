...
public class LiveControl implements RtspClientCallback, PlayerDisplayCB {
	...
	private void startRtsp() {
		...
		mRtspEngineIndex = mRtspHandler.createRtspClientEngine(this,
				RtspClient.RTPRTSP_TRANSMODE);
		...
		boolean ret = mRtspHandler.startRtspProc(mRtspEngineIndex, mUrl,
				mDeviceUserName, mDevicePassword);
		if (!ret) {
			int errorCode = mRtspHandler.getLastError();
			DebugLog.error(TAG, "startRtsp():: errorCode is R" + errorCode);
			if (null != mLiveCallBack) {
	...
	/*
	 * handle - - 引擎id dataType - -
	 * 数据类型，决定data数据的类型,包括DATATYPE_HEADER和DATATYPE_STREAM两种类型 data -
	 * -回调数据,分为：header数据和stream数据，由datatype作区分，header用于初始化播放库 length - - data
	 * 数据的长度 timeStamp - - 时间戳（保留） packetNo - -rtp包号（保留） useId - -
	 * 用户数据，默认就是引擎id与handle相同
	 */
		...
		case RtspClient.DATATYPE_HEADER:
			...
			if (!ret) {
				if (null != mLiveCallBack) {
					...
					DebugLog.error(TAG,
							"onDataCallBack():: mLiveCallBack is null");
				}
			} else {
	...
	private boolean startPlayer(byte[] data, int len) {
		if (null == data || 0 == len) {
			DebugLog.error(TAG,
					"startPlayer() Stream data error data is null or len is 0");
			return false;
		}
		...
		if (!ret) {
			...
			mPlayerPort = -1;
			DebugLog.error(TAG,
					"startPlayer(): Player setStreamOpenMode failed! errorCord is P"
							+ tempErrorCode);
			return false;
		}
		...
		if (!ret) {
			DebugLog.error(
					TAG,
					"startPlayer() mPlayerHandle.openStream failed!" + "Port: "
							+ mPlayerPort + "ErrorCode is P "
							+ mPlayerHandler.getLastError(mPlayerPort));
			return false;
		}
		...
		if (!ret) {
			DebugLog.error(
					TAG,
					"startPlayer() mPlayerHandle.play failed!" + "Port: "
							+ mPlayerPort + "PlayView Surface: "
							+ surfaceHolder + "errorCode is P"
							+ mPlayerHandler.getLastError(mPlayerPort));
			return false;
		}
	...
	public boolean capture(String filePath, String picName) {
		...
		if (LIVE_PLAY != mLiveState) {
			mLiveCallBack
					.onMessageCallback(ConstantLive.CAPTURE_FAILED_NPLAY_STATE);
			return false;
		}
	...
	private byte[] getPictureOnJPEG() {
		...
		if (!ret) {
			DebugLog.error(TAG,
					"getPictureOnJPEG():: mPlayerHandler.getJPEG() return false");
			return null;
		}
	...
	private void processStreamData(byte[] data, int len) {
		if (null == data || 0 == len) {
			DebugLog.error(TAG,
					"processStreamData() Stream data is null or len is 0");
			return;
		}
		if (null != mPlayerHandler) {
	...
	public void onMessageCallBack(int handle, int opt, int param1, int param2,
		...
		if (opt == RtspClient.RTSPCLIENT_MSG_CONNECTION_EXCEPTION) {
			stop();
			DebugLog.error(TAG,
					"onMessageCallBack():: rtsp connection exception");
			if (connectNum > 3) {
				DebugLog.error(TAG,
						"onMessageCallBack():: rtsp connection more than three times");
				connectNum = 0;
			} else {
	...
	public Calendar getOSDTime() {
		...
		if (!ret) {
			DebugLog.error(TAG,
					"getOSDTime(): getSystemTime() fail errorCode is "
							+ mPlayerHandler.getLastError(mPlayerPort));
	...
	public void onDisplay(int arg0, byte[] arg1, int arg2, int arg3, int arg4,
		...
		if (LIVE_PLAY != mLiveState) {
			...
			if (null != mLiveCallBack) {
				mLiveCallBack
						.onMessageCallback(ConstantLive.PLAY_DISPLAY_SUCCESS);
			} else {
